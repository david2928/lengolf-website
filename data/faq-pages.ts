...
export async function getIndoorGolfCostContent(catalog?: PricingCatalog | null): Promise<FaqContent> {
  ...
  const morningWD = bayRates[0]?.weekday ?? '500 THB'
  const afternoonWD = bayRates[1]?.weekday ?? '700 THB'
  const morningWE = bayRates[0]?.weekend ?? '700 THB'
  const afternoonWE = bayRates[1]?.weekend ?? '900 THB'

  const morningWDNum = parseInt(morningWD.replace(/[^0-9]/g, ''), 10)
  const perPerson = isNaN(morningWDNum) ? '100 THB' : formatThb(Math.round(morningWDNum / 5))
  ...
  return {
    ...base,
    answer_intro: `Indoor golf in Bangkok typically costs 500–1,000 THB per hour, depending on the venue, time of day, and day of the week. At LENGOLF, simulator bay rental starts at ${morningWD} per hour for up to 5 people — that's just ${perPerson} per person for a group. Free standard golf clubs are included with every booking.`,
