...
export const explainerPages: ExplainerSeoPage[] = [
  // ─── Page 1: What Is a Golf Simulator and How Does It Work? ───
  {
    ...
    content: {
      ...
      comparison_table: [
        ...
        { feature: 'Cost per person (group of 4)', simulator: '~138–238 THB/hr', real_golf: '2,500–4,000+ THB per round' },
  ...
  // invented; readers are told to verify with their airline.
  {
    ...
    content: {
      ...
      comparison_table: [
        { feature: 'タイ国際航空', simulator: '○ — 無料の1個としてカウント', real_golf: '米国・カナダ路線 150米ドル/個、その他はゾーン別料金' },
        { feature: 'シンガポール航空', simulator: '○ — 重量枠内', real_golf: '一律6kg換算の特例（超過15kgまで）' },
        { feature: 'キャセイパシフィック', simulator: '○ — 個数枠内', real_golf: '一律6kg換算の特例（超過15kgまで）' },
        { feature: 'エミレーツ航空', simulator: '○ — 重量枠内', real_golf: '通常のkg単位超過料金' },
        { feature: 'カタール航空', simulator: '○ — 重量枠内', real_golf: '通常料金（オンライン事前購入で最大20%割引）' },
        { feature: 'LCC（エアアジア、スクート等）', simulator: '× — 予約時に有料追加', real_golf: '別料金。空港での当日追加は大幅に割高' },
      ],
    },
  },
  ...
  // invented carrier-specific fee figures; verify-with-airline disclaimers kept.
  {
    ...
    content: {
      ...
      col_b_label: '超出免费额度时',
      comparison_table: [
        { feature: '泰国国际航空', simulator: '是 — 按一件免费行李计算', real_golf: '美国／加拿大航线150美元／件，其他按区域计费' },
        { feature: '新加坡航空', simulator: '是 — 在重量额度内', real_golf: '固定6公斤计费优惠（超额最多15公斤）' },
        { feature: '国泰航空', simulator: '是 — 在件数额度内', real_golf: '固定6公斤计费优惠（超额最多15公斤）' },
        { feature: '阿联酋航空', simulator: '是 — 在重量额度内', real_golf: '普通每公斤超额费率' },
        { feature: '卡塔尔航空', simulator: '是 — 在重量额度内', real_golf: '普通费率（在线提前购买最高享20%折扣）' },
        { feature: '廉价航空（亚洲航空、酷航等）', simulator: '否 — 需在订票时加购', real_golf: '单独收费，机场现场加购费率高得多' },
      ],
    },
  },
  ...
  // the /golf-course-club-rental related_slug carries the funnel.
  {
    ...
    content: {
      intro:
        '방콕은 세계에서도 손꼽히는 가성비 골프를 즐길 수 있는 곳이에요. 잘 관리된, 캐디가 함께하는 코스를 카트까지 포함해 라운딩해도, 서구 많은 퍼블릭 코스의 저렴한 라운딩보다 싸게 끝나요. 다만 태국에서 "그린피"가 총액을 뜻하는 경우는 드물어요 — 요금 구조를 이해해 두면 정산할 때 놀랄 일이 없어요.',
      sections: [
        {
