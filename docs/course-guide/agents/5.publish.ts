 ...
 * Converts an approved course JSON file from data/4.approved/ into a TypeScript
 * data object and writes it to data/golf-courses/<region>/<slug>.ts
 *
 * This script is run by Claude on your behalf after you approve a batch.
 * It contains NO AI calls — it is a pure mechanical formatter.
 *
 * Usage (Claude runs this for you):
 *   npx tsx docs/course-guide/agents/5.publish.ts <region> <slug>
 *   npx tsx docs/course-guide/agents/5.publish.ts <region> --all
 */
...
if (!region || !slugArg) {
  console.error('Usage:')
  console.error('  npx tsx docs/course-guide/agents/5.publish.ts <region> <slug>')
  console.error('  npx tsx docs/course-guide/agents/5.publish.ts <region> --all')
  process.exit(1)
}
...
function buildSchemaMarkup(fields: Record<string, Record<string, unknown>>, slug: string, region: string): string {
  ...
  const amenityFeatures = amenities
    ...
    .map(
      (a) =>
        `{ "@type": "LocationFeatureSpecification", "name": "${a.name}", "value": ${v(fields[a.field])} }`
    )
    .join(', ')
  ...
  return JSON.stringify(
    {
      ...
      '@type': 'GolfCourse',
      name: v(fields['name']),
      url: `https://len.golf/golf-courses/${region}/${slug}`,
      ...
      priceRange: '฿฿฿',
      sameAs: sameAs.length ? sameAs.map((s) => s.replace(/^"|"$/g, '')) : undefined,
...
export const course: GolfCourse = {
  ...
  locales: {
    en: {
      ...
      meta_description: \`${escapeForTs(String(v(fields['name']) ?? ''))} green fees, course overview, tips, and how to arrange golf club rentals delivered to your Bangkok hotel.\`,
    },
    ko: null,
    zh: null,
    ja: null,
...
function publishCourse(region: string, slug: string): void {
  ...
  if (!fs.existsSync(approvedPath)) {
    console.error(`\n❌  No approved course found at data/4.approved/${region}/${slug}.json`)
    process.exit(1)
  }
...
function main() {
  if (slugArg === '--all') {
    ...
    if (!files.length) {
      console.error(`❌  No JSON files in data/4.approved/${region}/`)
      process.exit(1)
    }
